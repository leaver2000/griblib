...
class MosaicTile(object):
    ...
    def read_mosaic_binary(self, full_path_and_filename, verbose=False):
        ...
        f.seek(0)
        fileobj = np.fromstring(f.read(80 + 4 * self.nz + 82 + 4 * NR + 2 * self.nlon * self.nlat * self.nz), dtype=dt)
        f.close()
        ...
        self.Longitude, self.Latitude = np.meshgrid(lon, lat)
        self._get_tile_number()
        self.Height = 1.0 * fileobj["Height"][0] / fileobj["z_scale"][0] / ALTITUDE_SCALE_FACTOR
        if self.nz == 1:
    ...
    def read_mosaic_grib(
        ...
        self.Filename = (
            os.path.basename(filename[0]) if not isinstance(filename, six.string_types) else os.path.basename(filename)
        )
        self.Variables = [DEFAULT_VAR]
        gribfile = MosaicGrib(
            filename,
    ...
    def diag(self, verbose=False):
        ...
        print("Min, Max Latitude =", np.min(self.Latitude), np.max(self.Latitude))
        print("Min, Max Longitude =", np.min(self.Longitude), np.max(self.Longitude))
    ...
    def output_composite(self, full_path_and_filename=DEFAULT_FILENAME, var=DEFAULT_VAR, verbose=False):
        """
    ...
    def _get_tile_number(self):
        ...
        self.Tile = "?"
        if self.Version == 1:
            if _are_equal(self.StartLat, 55.0) and _are_equal(self.StartLon, -130.0):
                self.Tile = "1"
            elif _are_equal(self.StartLat, 55.0) and _are_equal(self.StartLon, -110.0):
                self.Tile = "2"
            elif _are_equal(self.StartLat, 55.0) and _are_equal(self.StartLon, -90.0):
                self.Tile = "3"
            elif _are_equal(self.StartLat, 55.0) and _are_equal(self.StartLon, -80.0):
                self.Tile = "4"
            elif _are_equal(self.StartLat, 40.0) and _are_equal(self.StartLon, -130.0):
                self.Tile = "5"
            elif _are_equal(self.StartLat, 40.0) and _are_equal(self.StartLon, -110.0):
                self.Tile = "6"
            elif _are_equal(self.StartLat, 40.0) and _are_equal(self.StartLon, -90.0):
                self.Tile = "7"
            elif _are_equal(self.StartLat, 40.0) and _are_equal(self.StartLon, -80.0):
                self.Tile = "8"
        elif self.Version == 2:
            if _are_equal(self.StartLat, 54.995) and _are_equal(self.StartLon, -129.995):
                self.Tile = "1"
            elif _are_equal(self.StartLat, 54.995) and _are_equal(self.StartLon, -94.995):
                self.Tile = "2"
            elif _are_equal(self.StartLat, 37.495) and _are_equal(self.StartLon, -129.995):
                self.Tile = "3"
            elif _are_equal(self.StartLat, 37.495) and _are_equal(self.StartLon, -94.995):
                self.Tile = "4"
    ...
    def _construct_header(self):
        ...
        Height = np.int32(self.Height * DEFAULT_Z_SCALE * ALTITUDE_SCALE_FACTOR).tostring()  # km to m
    ...
    def _subsection_in_latitude(self, latrange=None):
        if latrange and np.size(latrange) == 2:
            ...
            condition = np.logical_or(temp < np.min(latrange), temp > np.max(latrange))
    ...
    def _subsection_in_longitude(self, lonrange=None):
        if lonrange and np.size(lonrange) == 2:
            ...
            condition = np.logical_or(temp < np.min(lonrange), temp > np.max(lonrange))
            indices = np.where(condition)
    ...
    def _subsection_in_height(self, zrange=None):
        if zrange and np.size(zrange) == 2:
            temp = self.Height[:]
            condition = np.logical_or(temp < np.min(zrange), temp > np.max(zrange))
            indices = np.where(condition)
...
class MosaicGrib(object):
    ...
    def read_grib_list(
        ...
        nclist = []
        gblist = []
        for grib in file_list if not isinstance(file_list, six.string_types) else [file_list]:
            try:
                ...
                if IMPORT_FLAG:
                    ...
                    if latrange is None and lonrange is None:
                        command = wgrib2_path + wgrib2_name + " " + grib + " -netcdf " + nc_path + gribf + ".nc"
    ...
    def format_grib_data(self):
        ...
        for i, gr in enumerate(self.gblist):
            ...
            if i == 0:
                ...
                self.Latitude = lat
                self.LatGridSpacing = np.round(np.abs(self.Latitude[0, 0] - self.Latitude[1, 0]), decimals=2)
                self.LonGridSpacing = np.round(np.abs(self.Longitude[0, 1] - self.Longitude[0, 0]), decimals=2)
                self.StartLat = np.max(self.Latitude)
                self.StartLon = np.min(self.Longitude)
                dtgrb = datetime.datetime.strptime(str(grb["dataDate"]) + str(grb["dataTime"]), "%Y%m%d%H%M")
                self.Time = (dtgrb - datetime.datetime(1970, 1, 1)).total_seconds()
                mrefl3d = np.zeros(
                    (np.size(self.gblist), np.shape(self.Latitude)[0], np.shape(self.Longitude)[1]), dtype="float"
                )
    ...
    def format_netcdf_data(self):
        ...
        mrefl3d = np.zeros(
            (np.size(height), np.size(self.nclist[0].latitude), np.size(self.nclist[0].longitude)), dtype="float"
        )
        ...
        self.Longitude, self.Latitude = np.meshgrid(self.nclist[0].longitude, self.nclist[0].latitude[::-1])
        ...
        self.LatGridSpacing = np.abs(self.nclist[0].latitude[0] - self.nclist[0].latitude[1])
        self.LonGridSpacing = np.abs(self.nclist[0].longitude[0] - self.nclist[0].longitude[1])
...
class MosaicStitch(MosaicTile):
    ...
    def help(self):
        ...
        print("Instance = stitch_mosaic_tiles(map_array=map_array,", "direction=direction)")
    ...
    def stitch_ns(self, n_tile=None, s_tile=None, verbose=False):
        ...
        if n_tile.nlon != s_tile.nlon:
            print(method_name + "(): Grid size in Longitude does not match,", "fix this before proceeding")
            return
        inlat, index = self._stitch_radar_variables(n_tile, s_tile, verbose, ns_flag=True)
        if index == 0:
    ...
    def stitch_we(self, w_tile=None, e_tile=None, verbose=False):
        ...
        if w_tile.nlat != e_tile.nlat:
            print(method_name + "(): Grid size in Latitude does not match,", "fix this before proceeding")
            return
    ...
    def _stitch_metadata(self, a_tile=None, b_tile=None, index=None, ns_flag=True):
        """
        ns_flag=True means N-S stitch, False means W-E stitch.
        Uses np.append() to stitch together.
        """
        if ns_flag:
            self.Longitude = np.append(a_tile.Longitude, b_tile.Longitude[:index, :], axis=0)
            self.Latitude = np.append(a_tile.Latitude, b_tile.Latitude[:index, :], axis=0)
            if a_tile.Version == 1:
            ...
            self.Longitude = np.append(a_tile.Longitude[:, :index], b_tile.Longitude, axis=1)
            self.Latitude = np.append(a_tile.Latitude[:, :index], b_tile.Latitude, axis=1)
            if a_tile.Version == 1:
    ...
    def _stitch_radar_variables(self, a_tile=None, b_tile=None, verbose=False, ns_flag=True):
        """
        ns_flag=True means N-S stitch, False means W-E stitch.
        Uses np.append() to stitch together.
...
class MosaicDisplay(object):
    ...
    def plot_horiz(
        ...
        if basemap is None:
            m = self._create_basemap_instance(latrange, lonrange, resolution, area_thresh)
        else:
            m = basemap
        if embellish:
            m = self._add_gridlines_if_desired(m, parallels, meridians, linewidth, latrange, lonrange, show_grid)
    ...
    def plot_vert(
        ...
        vcut, xvar, xrange, xlabel, tlabel = self._get_vertical_slice(var, lat, lon, xrange, xlabel, verbose)
        if vcut is None:
            return
        # Plot details
        if not title:
        ...
        ax, cs = self._plot_vertical_cross_section(
            ax, vcut, xvar, xrange, xlabel, zrange, zlabel, clevs, cmap, title, mappable=True
        )
        if colorbar_flag:
    ...
    def three_panel_plot(
        ...
        if lat is None or lon is None:
            print(method_name + "(): Need both constant latitude and", "constant longitude for slices")
            if verbose:
                _method_footer_printout()
            return
        fig = plt.figure()
    ...
    def _create_basemap_instance(self, latrange=None, lonrange=None, resolution="l", area_thresh=10000):
    ...
    def _add_gridlines_if_desired(
        ...
        if show_grid:
            # Draw parallels
            vparallels = np.arange(np.floor(np.min(latrange)), np.ceil(np.max(latrange)), parallels)
            m.drawparallels(vparallels, labels=[1, 0, 0, 0], fontsize=10, linewidth=linewidth)
            # Draw meridians
            vmeridians = np.arange(np.floor(np.min(lonrange)), np.ceil(np.max(lonrange)), meridians)
            m.drawmeridians(vmeridians, labels=[0, 0, 0, 1], fontsize=10, linewidth=linewidth)
        return m

    def _add_crosshairs(self, m=None, lat=None, lon=None, xrange_b=None, xrange_c=None):
    ...
    def _get_vertical_slice(self, var=DEFAULT_VAR, lat=None, lon=None, xrange=None, xlabel=None, verbose=False):
        ...
        if lat is None and lon is None:
            ...
            if lon is None:
                if self.mosaic.nlon <= 1:
                    ...
                    return fail
                if verbose:
                    print("Plotting vertical cross-section thru", lat, "deg Latitude")
                if not xrange:
                    xrange = [np.min(self.mosaic.Longitude), np.max(self.mosaic.Longitude)]
                if not xlabel:
                    xlabel = "Longitude (deg)"
                vcut, xvar, tlabel = self._get_constant_latitude_cross_section(var, lat)
            if lat is None:
                if self.mosaic.nlat <= 1:
                    ...
                    return fail
                if verbose:
                    print("Plotting vertical cross-section thru", lon, "deg Longitude")
                if not xrange:
                    xrange = [np.min(self.mosaic.Latitude), np.max(self.mosaic.Latitude)]
                if not xlabel:
                    xlabel = "Latitude (deg)"
                vcut, xvar, tlabel = self._get_constant_longitude_cross_section(var, lon)
    ...
    def _parse_lat_tlabel(self, lat):
        if lat > np.max(self.mosaic.Latitude):
            ...
            print("Outside domain, plotting instead thru", lat, " deg Latitude")
    ...
    def _get_constant_latitude_cross_section(self, var=DEFAULT_VAR, lat=None):
        ...
        index = np.round(np.abs(lat - self.mosaic.StartLat) / self.mosaic.LatGridSpacing)
    ...
    def _parse_lon_tlabel(self, lon):
        if lon > np.max(self.mosaic.Longitude):
            ...
            print("Outside domain, plotting instead thru", lon, " deg Longitude")
    ...
    def _get_constant_longitude_cross_section(self, var=DEFAULT_VAR, lon=None):
        ...
        index = np.round(np.abs(lon - self.mosaic.StartLon) / self.mosaic.LonGridSpacing)
        index = np.int32(index)
...
def stitch_mosaic_tiles(map_array=None, direction=None, verbose=False):
    ...
    # 2-D stitching in N-S and W-E
        ...
        if verbose:
            print("Sent a 2-D matrix, attempting to stitch in both", "N-S and E-W directions")
        # Tile number fail
...
def epochtime_to_string(epochtime=None, use_second=False):
    ...
    try:
        if use_second:
            time_string = time.strftime("%m/%d/%Y %H:%M:%S UTC", time.gmtime(epochtime))
        else:
            time_string = time.strftime("%m/%d/%Y %H:%M UTC", time.gmtime(epochtime))
    except:
        time_string = ""
    return time_string
...
def _stitch_1d_array_we(map_array=None, verbose=False, method_name="stitch_mosaic_tiles"):
    result = MosaicStitch()
    if verbose:
    ...
    for i in np.arange(np.shape(map_array)[0]):
        if not hasattr(map_array[i], DEFAULT_VAR):
            ...
            return False
        if i == 0:
            result.stitch_we(w_tile=map_array[i], e_tile=map_array[i + 1], verbose=verbose)
        if 1 <= i < np.shape(map_array)[0] - 1:
            result.stitch_we(w_tile=result, e_tile=map_array[i + 1], verbose=verbose)
    if verbose:
        _print_method_done()
        _method_footer_printout()
    return result
...
def _stitch_1d_array_ns(map_array=None, verbose=False, method_name="stitch_mosaic_tiles"):
    result = MosaicStitch()
    if verbose:
    ...
    for i in np.arange(np.shape(map_array)[0]):
        if not hasattr(map_array[i], DEFAULT_VAR):
            ...
            return False
        if i == 0:
            result.stitch_ns(n_tile=map_array[i], s_tile=map_array[i + 1], verbose=verbose)
        if 1 <= i < np.shape(map_array)[0] - 1:
            result.stitch_ns(n_tile=result, s_tile=map_array[i + 1], verbose=verbose)
    if verbose:
        _print_method_done()
        _method_footer_printout()
    return result
...
def _stitch_2d_array(map_array=None, verbose=False, method_name="stitch_mosaic_tiles"):
    ...
    for i in np.arange(np.shape(map_array)[1]):
        if not hasattr(map_array[0][i], DEFAULT_VAR) or not hasattr(map_array[1][i], DEFAULT_VAR):
            _print_missing_a_tile(method_name)
            return False
        tmp = MosaicStitch()
        ns_stitches.append(tmp)
        ns_stitches[i].stitch_ns(n_tile=map_array[0][i], s_tile=map_array[1][i], verbose=verbose)
        if i == 0:
            result = ns_stitches[i]
        if i >= 1:
            result.stitch_we(w_tile=result, e_tile=ns_stitches[i], verbose=verbose)
    return result
